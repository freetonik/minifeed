...
export async function enqueueUpdateAllFeeds(env: Bindings) {  // usually triggered by CRON
    type FeedsRowPartial = {
        feed_id: number;
        rss_url: string;
    }
    const { results: feeds } = await env.DB.prepare("SELECT feed_id, rss_url FROM feeds").all<FeedsRowPartial>();
...
export async function enqueueScrapeAllItemsOfFeed(env: Bindings, feed_id: number) {
    ...
    const { results: items } = await env.DB.prepare("SELECT item_id, url FROM items WHERE feed_id = ?").bind(feed_id).all<ItemsRowPartial>();
