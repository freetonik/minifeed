...
export const handleListsSingle = async (c: Context) => {
    ...
    const [list, list_items] = await c.env.DB.batch([
        ...
        c.env.DB.prepare(
            `SELECT items.item_sqid, items.title, items.url, items.pub_date, feeds.title as feed_title, feeds.feed_sqid, items.description
    ...
    <p style="margin: 0.75em 0 2em;">list by <a href="/users/${listEntry.username}">@${listEntry.username}</a></p>`;
    ...
    if (userId === listEntry.user_id) {
        inner += `<p>
            <button class="button" hx-confirm="Are you sure?" hx-swap="outerHTML transition:true" hx-post="/lists/${listSqid}/delete">Delete list</button>
        </p>`;
    }
