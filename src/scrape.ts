...
export async function scrapeItem(env: Bindings, item_id: number) {
    const { results: items } = await env.DB.prepare("SELECT url, description FROM items WHERE item_id = ?").bind(item_id).all();
    ...
    try {
        ...
        if (!item_description || item_description.length < 5) item_description = stripTags(truncate(content, 500));
        await env.DB.prepare("UPDATE items SET content_html_scraped = ?, description = ? WHERE item_id = ?").bind(content, item_description, item_id).run();
    } catch (err: any) {
